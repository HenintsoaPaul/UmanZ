...
const idTalent = computed(() => route.params.idTalent);
...
async function loadTalentEvaluations(idTalent: number) {
  try {
    const apiUrl: string = useRuntimeConfig().public.apiUrl as string;
    const response = await axios.get(`${apiUrl}/evaluation/note/${idTalent}`);
...
onMounted(() => {
  loadTalentEvaluations(Number(idTalent));
})
